...
class NCProtocol(Protocol):
    def __init__(self, factory, state="GETHELLO", kind="LISTENER", cache = None):
    ...
    def dataReceived(self, data):
        for line in data.splitlines():
            ...
            if self.state in ["GETHELLO", "SENTHELLO"]:
                ...
                elif envelope['msgtype'] == 'cache':
                    self.handle_CACHE(line)

    def send_CACHE(self):
        _print(" [>] send CACHE   to", self.remote_nodeid, "at", self.remote_ip)
        cache_msg = messages.create_cache(self.nodeid, self.cache)
        self.write(cache_msg)

    def handle_CACHE(cache):
        cache_data = messages.read_message(cache)['cache_data']
        recv_cache = pickle.loads(cache_data)
        _print(" [<] Recieved cache list from peer " + self.remote_nodeid)

        urlInRecvCache = recv_cache.returnKeys()
        for url in urlInRecvCache:
            if not self.cache.existingUrl(url):
                self.cache.cachePush(url, recv_cache[url])
